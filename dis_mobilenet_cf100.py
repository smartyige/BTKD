...
from dis_model import h_resnet50
from dis_model import MobileNetV2
...
if __name__ == '__main__':
    ...
    studentNet = MobileNetV2(num_classes=100)
    # studentNet.load_state_dict(torch.load("./studentNet.pth"))
    ...
    best_acc = 0.0
    save_path = './pth/dis_mobilenetv2_cf100.pth'
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            for val_data in val_loader:
                ...
                y_pre = torch.max(outputs, dim=1)[1]
                acc += torch.eq(y_pre, val_labels.to(device)).sum().item()
        ...
        print('dis_mobilenetv2_cf100_T, Finished Training    betacc = %.4f' % best_acc)
        logger.info('dis_mobilenetv2_cf100_T, Finished Training    betacc = %.4f' % best_acc)