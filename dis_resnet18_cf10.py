import torch
from dataload_cf10 import train_loader,val_loader,train_dataset,val_dataset
...
tips = '_dis_resnet18_cf10'
save_log(tips)

####################################去错+校正#############################################

if __name__ == '__main__':
    Teacher = h_resnet50(num_classes=10)
    device = torch.device('cuda:0')
    teacherNet = Teacher
    teacherNet.load_state_dict(torch.load("./pth/h_resnet50_cf10.pth"))
    teacherNet.eval()
    # teacherNet.train(mode=False)
    teacherNet = teacherNet.to(device)

    studentNet = h_resnet18(num_classes=10)
    # studentNet.load_state_dict(torch.load("./pth/h_resnet18_cf10.pth", map_location=device))
